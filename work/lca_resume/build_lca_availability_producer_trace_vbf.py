#!/usr/bin/env python3
"""Build proven LCA bypasses with FD22 format-5 availability-producer telemetry."""
...
DEFAULT_OUTPUT=ROOT/'CV6T-14C217-AR_LCA_AVAIL_PRODUCER_TRACE.VBF'
...
def selftest():
 try:
  ...
  audit(s,base.parse_vbf_bytes(a.output,Path('availability-producer-selftest.vbf')),c);print('SELFTEST: ALL PASS');return 0
...
def main():
 ...
 try:
  ...
  print(f'wrote: {args.output}\nsha256: {base.sha256(r.output)}');print('control patches: dispatcher gate + phase-1 entry bypass (unchanged from joint4)');print('FD22 format 5: '+','.join(f'X:${x:04X}' for x in TRACE_SOURCES));print(f'word A/B: {r.new_word_a:04X}/{r.new_word_b:04X}');print(f'block CRC/file checksum: {r.new_block_crc:04X}/{r.new_file_checksum:08X}');return 0
