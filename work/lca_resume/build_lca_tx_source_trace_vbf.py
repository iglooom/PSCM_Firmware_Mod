#!/usr/bin/env python3
"""Build proven LCA bypasses with FD22 format-4 TX-source telemetry."""
...
DEFAULT_OUTPUT=ROOT/'CV6T-14C217-AR_LCA_TX_SOURCE_TRACE.VBF'
TRACE_SOURCES=(0x2DDE,0x2DB9,0x2D53,0x2DAF,0x7EBB,0x3F5B,0x220F)
def make_handler():
 w=[0xE084,0xD0B6];off=1
...
def selftest():
 try:
  ...
  audit(s,base.parse_vbf_bytes(a.output,Path('tx-source-selftest.vbf')),c);print('SELFTEST: ALL PASS');return 0
...
def main():
 ...
 try:
  ...
  print(f'wrote: {args.output}\nsha256: {base.sha256(r.output)}');print('control patches: dispatcher gate + phase-1 entry bypass (unchanged from joint4)');print('FD22 format 4: '+','.join(f'X:${x:04X}' for x in TRACE_SOURCES));print(f'word A/B: {r.new_word_a:04X}/{r.new_word_b:04X}');print(f'block CRC/file checksum: {r.new_block_crc:04X}/{r.new_file_checksum:08X}');return 0
