#!/usr/bin/env python3
"""Independent verifier for LCA availability-chain trace VBF."""
import hashlib,struct,sys
from pathlib import Path
import verify_fd22_snapshot_vbf_independent as ind
ROOT=Path(__file__).resolve().parents[2];STOCK=ROOT/'CV6T-14C217-AR.VBF';CAL=ROOT/'CV6T-14C218-AX.VBF';OUT=ROOT/'CV6T-14C217-AR_LCA_AVAIL_TRACE.VBF'
HASHES={STOCK.name:'cc68d97b1b8ca3ff9449d8e42ffa6ec16280f640f8b7928af4080b40592b96f5',CAL.name:'6fdc0ad81727fd39db98a486f2ec2b4de928100138fc93b8c3e5c2a8e307c5f7',OUT.name:'0a1ce0dc4135c229393043bfeb47a473c1eef32d368fb0ad340654f21762c9fe'}
SOURCES=(0x2DDE,0x2DB9,0x2D53,0x2DAF,0x2213,0x226C,0x220F);OEM=(0x4C06,0xA207,0xF07C,0x0904,0x4C01,0xA203,0xE684,0x2DDE,0xA902,0xE680,0x2DDE);OPEN=(0x4C06,0xA207,0xE700,0xE700,0xE700,0xE700,0xE684,0x2DDE,0xA902,0xE680,0x2DDE)
def req(v,m):
 if not v:raise ind.VerifyError(m)
def verify_handler(v):
 w=ind.p_words(v,ind.CAVE_P,60);req(w[:2]==(0xE083,0xD0B6),'format mismatch');src=[];offs=[]
...
def verify():
 ...
 req({a for a,_,_ in diff}<=allowed,'unexpected payload difference');b0,b1=o.blocks[0],o.blocks[1];sa=struct.unpack_from('<H',b1.payload,ind.CRC_A_BLOCK1_END)[0];ca=ind.crc16_mcrf4xx(b0.payload+b1.payload[ind.CRC_A_BLOCK1_START:ind.CRC_A_BLOCK1_END]);req(sa==ca==0x7803,'A mismatch');image=ind.make_linear_image(o,c);sb=struct.unpack_from('<H',image,ind.CHECK_B_FLASH)[0];cb=ind.sum_le16(image[:ind.CHECK_B_FLASH]);req(sb==cb==0x2E3A,'B mismatch')
 print('Independent LCA availability-trace verification');print('SHA-256:',HASHES[OUT.name]);print('Container/block/file integrity: PASS');print(f'Internal A/B: {sa:04X}/{sb:04X} PASS');print('Control patches: byte-identical dispatcher + phase-entry bypasses from joint4');print('State test, B==3 condition, phase/code writes: RETAINED');print('FD22: format 3; sources '+','.join(f'X:${x:04X}' for x in SOURCES));print(f'Payload differences: {len(diff)} bytes, all classified');print('READY FOR CONTROLLED TEST')
def main():
 try:verify();return 0
