#!/usr/bin/env python3
"""Build HV6T-14C217-AC_LCA_ENABLED.VBF: the five-edit LCA enabler relocated
from CV6T-14C217-AR to the HV6T-14C217-AC revision.

HV6T-14C217-AC is an isomorphic later revision of the same PSCM application:
identical block topology, identical checksum algorithms (word A CRC16/MCRF4XX
gapped, word B sum16le whole-module, START 0x1800), and the exact same lane-assist
code — only the flash addresses and a fixed RAM-cell delta differ. Each of the
five patch sites was relocated structurally (opcode-pattern search, cell-wildcard)
and its stock words re-asserted here.
...
OUTPUT = ROOT / "HV6T-14C217-AC_LCA_ENABLED.VBF"

# whole-file SHA-256 of the OEM inputs this build is bound to
...
# Context that must survive verbatim (a mismatch => wrong image / bad relocation)
GUARDS = (
...
# The two dispatch tables: only entry 5 (byte offset table+0x14) may move.
D1_TABLE = 0x3A18C
D2_TABLE = 0x3A28E
...
def verify_oem(stock, cal):
    ...
    for addr, want in OEM_BLK_SHA.items():
        got = _sha(stock.block_at(addr).data)
        if got != want:
    ...
    # confirm the checksum algorithms reproduce the stored OEM words
...
def selftest():
    try:
        ...
        parsed = base.parse_vbf_bytes(first.output, Path("hv6t-selftest.vbf"))
        n = audit(stock, parsed, cal)
        print(f"SELFTEST: ALL PASS ({n} bytes changed = patch words + 2 checksums)")
        return 0
