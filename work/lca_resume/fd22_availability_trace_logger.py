#!/usr/bin/env python3
"""Log FD22 format-3 PSCM availability-chain telemetry to CSV."""
from __future__ import annotations
import argparse,csv,datetime as dt,importlib.util,time
from pathlib import Path
REQUEST='22FD22';FORMAT=3
FIELD_NAMES=('lane_state','per_state_code','torque_accumulator','state_machine_phase','candidate_source','candidate_copy','candidate_consumer')
SOURCES=('X:$2DDE','X:$2DB9','X:$2D53','X:$2DAF','X:$2213','X:$226C','X:$220F')
...
def selftest():
 p=bytes.fromhex('03 0004 0005 FFFE 0006 0003 0002 0001');d=decode_response(bytes.fromhex('62FD22')+p);assert [d[n+'_signed'] for n in FIELD_NAMES]==[4,5,-2,6,3,2,1]
 try:decode_response(bytes.fromhex('62FD22')+bytes([2])+p[1:])
 except ValueError:pass
 else:raise AssertionError('format 2 accepted')
 print('SELFTEST PASS: format 3; '+', '.join(f'{n}={s}' for n,s in zip(FIELD_NAMES,SOURCES)));return 0
def main():
 ...
 out=Path(a.output or ('fd22_avail_'+dt.datetime.now().strftime('%Y%m%d_%H%M%S')+'.csv')).resolve();Ecu=load_ecu(a.vbflasher_dir);ecu=Ecu(a.iface,0x730,0x738,execute=True)
 def request(payload,timeout):return ecu.req(payload,timeout=timeout,pending_timeout=timeout,what='FD22 availability trace')
 def progress(r):
  if r['status']=='ok':print(f"\r{r['utc']} state={r['lane_state']:>2} code={r['per_state_code']:>2} acc={r['torque_accumulator_signed']:>6} phase={r['state_machine_phase']:>2} src={r['candidate_source']:>2} copy={r['candidate_copy']:>2} consumer={r['candidate_consumer']:>2}",end='',flush=True)
  else:print(f"\r{r['utc']} {str(r['status']).upper()}",end='',flush=True)
 print(f'FD22 format-3 logger -> {out}\nPress Ctrl-C to stop safely.');total='interrupted'
 try:
