#!/usr/bin/env python3
"""Build proven LCA bypasses with FD22 format-3 availability-chain telemetry."""
...
DEFAULT_OUTPUT=ROOT/'CV6T-14C217-AR_LCA_AVAIL_TRACE.VBF'
TRACE_SOURCES=(0x2DDE,0x2DB9,0x2D53,0x2DAF,0x2213,0x226C,0x220F)
def make_handler():
 w=[0xE083,0xD0B6];off=1
...
def audit(stock,built,cal):
 ...
 base.expect_bytes(new,base.POINTER_OFF,struct.pack('<2H',*base.NEW_POINTER_WORDS),'built pointer');base.expect_bytes(new,base.HANDLER_OFF,struct.pack('<60H',*TRACE_HANDLER),'built handler');base.expect_bytes(new,gatebase.GATE_ARM_OFF,struct.pack('<11H',*gatebase.OPEN_GATE_ARM),'built dispatcher gate');base.expect_bytes(new,entrybase.ENTRY_GUARD_OFF,struct.pack('<3H',*([0xE700]*3)),'built entry bypass')
 allowed=set(range(base.POINTER_OFF,base.POINTER_OFF+4))|set(range(base.HANDLER_OFF,base.HANDLER_OFF+120))|set(range(gatebase.GATE_GUARD_OFF,gatebase.GATE_GUARD_OFF+8))|set(range(entrybase.ENTRY_GUARD_OFF,entrybase.ENTRY_GUARD_OFF+6))|set(range(base.WORD_A_OFF,base.WORD_A_OFF+2))|set(range(base.WORD_B_OFF,base.WORD_B_OFF+2));changed={i for i,(a,z) in enumerate(zip(old,new)) if a!=z}
...
def selftest():
 try:
  ...
  audit(s,base.parse_vbf_bytes(a.output,Path('avail-selftest.vbf')),c);print('SELFTEST: ALL PASS');return 0
...
def main():
 ...
 try:
  ...
  print(f'wrote: {args.output}\nsha256: {base.sha256(r.output)}');print('control patches: dispatcher gate + phase-1 entry bypass (unchanged from joint4)');print('FD22 format 3: '+','.join(f'X:${x:04X}' for x in TRACE_SOURCES));print(f'word A/B: {r.new_word_a:04X}/{r.new_word_b:04X}');print(f'block CRC/file checksum: {r.new_block_crc:04X}/{r.new_file_checksum:08X}');return 0
