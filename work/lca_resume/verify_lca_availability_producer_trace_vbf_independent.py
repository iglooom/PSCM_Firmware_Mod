#!/usr/bin/env python3
"""Independent verifier for LCA availability-producer trace VBF."""
import hashlib,struct,sys
from pathlib import Path
import verify_fd22_snapshot_vbf_independent as ind
ROOT=Path(__file__).resolve().parents[2];STOCK=ROOT/'CV6T-14C217-AR.VBF';CAL=ROOT/'CV6T-14C218-AX.VBF';OUT=ROOT/'CV6T-14C217-AR_LCA_AVAIL_PRODUCER_TRACE.VBF'
HASHES={STOCK.name:'cc68d97b1b8ca3ff9449d8e42ffa6ec16280f640f8b7928af4080b40592b96f5',CAL.name:'6fdc0ad81727fd39db98a486f2ec2b4de928100138fc93b8c3e5c2a8e307c5f7',OUT.name:'db35a62e2fe7f79e1e3f331808a4c6ee50826db5c68e404828eb4aa3bf40bae2'}
...
def verify():
 ...
 req({a for a,_,_ in diff}<=allowed,'unexpected payload difference');b0,b1=o.blocks[0],o.blocks[1];sa=struct.unpack_from('<H',b1.payload,ind.CRC_A_BLOCK1_END)[0];ca=ind.crc16_mcrf4xx(b0.payload+b1.payload[ind.CRC_A_BLOCK1_START:ind.CRC_A_BLOCK1_END]);req(sa==ca==0x7AB1,'A mismatch');image=ind.make_linear_image(o,c);sb=struct.unpack_from('<H',image,ind.CHECK_B_FLASH)[0];cb=ind.sum_le16(image[:ind.CHECK_B_FLASH]);req(sb==cb==0x4790,'B mismatch')
 print('Independent LCA availability-producer-trace verification');print('SHA-256:',HASHES[OUT.name]);print('Container/block/file integrity: PASS');print(f'Internal A/B: {sa:04X}/{sb:04X} PASS');print('Control patches: byte-identical dispatcher + phase-entry bypasses from joint4');print('State test, B==3 condition, phase/code writes: RETAINED');print('FD22: format 5; sources '+','.join(f'X:${x:04X}' for x in SOURCES));print(f'Payload differences: {len(diff)} bytes, all classified');print('READY FOR CONTROLLED TEST')
def main():
 try:verify();return 0
